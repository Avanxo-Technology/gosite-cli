...
func (h *Handlers) homeCacheKey() string { return h.cacheKeyPrefix() + "home_html" }
...
func (h *Handlers) Home(c *echo.Context) error {
	html, cached, err := h.Cache.HTML(c.Request().Context(), h.homeCacheKey(), h.renderHome)
