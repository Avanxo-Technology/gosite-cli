...
// WithProject namespaces the SEO cache keys under the project name. The project
// name is not decoration: several projects share one Redis, and without it every
...
func (s *SEO) Purge(ctx context.Context, path string) error {
	keys := []string{s.keyPrefix + "defaults"}
	if path != "" {
		keys = append(keys, s.keyPrefix+"page:"+path)
	}
...
func (s *SEO) PurgeAll(ctx context.Context) error {
	return s.cache.PurgeGroup(ctx, s.keyPrefix)
}
